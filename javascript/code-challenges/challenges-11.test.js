...
const count = (target, input) => {
  const result = input.reduce((count, array) => {
    const occurrences = array.reduce((acc, num) => {
      if (num === target) {
        return acc + 1;
      }
      return acc;
...
const totalSum = (input) => {
  const sum = input.reduce((acc, array) => {
    const arraySum = array.reduce((total, num) => {
      return total + num;
    }, 0);
    return acc + arraySum;
  }, 0);
