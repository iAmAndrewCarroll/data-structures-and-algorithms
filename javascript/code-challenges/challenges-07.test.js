...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsArr.sort((a, b) => {
    // Convert the height values to numbers, because they are strings in the original objects
    const heightA = Number(a.height);
    const heightB = Number(b.height);

    // Since we want to sort from tallest to shortest, we return a positive value
    // when b is taller than a, and a negative value when a is taller than b
...
const removeThree = (idx, arr) => {
  ...
  // Removes three elements starting at the given index
  newArr.splice(idx, 3);
...
const howMuchPencil = (str) => {
  // Solution code here...
  let result = [];
  // The loop starts from 0 to the length of the string (inclusive)
  Array.from({ length: str.length + 1 }, (_, i) => result.push(str.slice(i)));
