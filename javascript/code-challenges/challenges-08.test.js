...
const sortByChildren = (charArray) => {
  ...
  return charArray.sort((a, b) => {
    // First sort by number of children
    if (a.children.length < b.children.length) {
      return -1;
    } else if (a.children.length > b.children.length) {
      return 1;
    }
    // If number of children are equal, sort by house name
    else {
      if (a.house < b.house) {
        return -1;
...
const containsW = (str) => {
  // The regex /w/ tests for the presence of the lowercase letter 'w' in the string.
  return /w/.test(str);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const isNum = (input) => {
  // The regex /\d/ tests for the presence of any digit (0-9) in the string.
  // We convert the input to a string using toString() method to handle both string and number inputs.
...
const containsWorld = (input) => {
  // The regex /world/ tests for the presence of the word 'world' in the string.
  // We convert the input to a string using toString() method to handle both string and number inputs.
  return /world/.test(input.toString());
};
...
const isCapitalized = (str) => {
  // The \b at both ends of the regex are word boundaries,
  // the [A-Z] matches any uppercase character,
  // the \w* matches any word character (equal to [a-zA-Z0-9_]) 0 or more times,
  // and the g flag at the end of the regex allows it to find all matches rather than stopping after the first match.
  let matches = str.match(/\b[A-Z]\w*\b/g);

  // The match method returns null if it finds no matches. So, we check for this case and return an empty array instead.
